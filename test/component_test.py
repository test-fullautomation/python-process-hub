...
# ProcessHub - Process lifecycle management hub
# --------------------------------------------------------------------------------------------------------------
...
def AnalyzeReturnedValues(EXPECTEDRETURN=None, actualReturned=None):
   ...
   if ( (EXPECTEDRETURN is None) and (actualReturned is None) ):
      ...
      sResult  = "ProcessHub returned None, but values are expected"
      listErrors.append(sResult)
   elif ( (EXPECTEDRETURN is None) and (actualReturned is not None) ):
      sResult  = "ProcessHub returned values, but values are not expected"
      listErrors.append(sResult)
   else:
      ...
      if nNrOfLinesReturned != nNrOfLinesExpected:
         bSuccess = False
         sResult  = f"ProcessHub value counter mismatch! Expected: {nNrOfLinesExpected} values, but returned: {nNrOfLinesReturned} values"
         listErrors.append(sResult)
         print("ProcessHub returned:")
         print()
   ...
   if len(listErrors) == 0:
      ...
      if EXPECTEDRETURN is None:
         sResult  = "No values returned from ProcessHub (like expected)."
      else:
         sResult  = "ProcessHub returned expected values."
   else:
      bSuccess = False
      sResult  = "ProcessHub did not return expected values."
...
def AnalyzeExceptions(EXPECTEDEXCEPTION=None, sException=None):
   ...
   if ( (EXPECTEDEXCEPTION is None) and (sException is None) ):
      ...
      sResult  = "ProcessHub threw no exception, but an exception is expected"
      listErrors.append(sResult)
   elif ( (EXPECTEDEXCEPTION is None) and (sException is not None) ):
      sResult  = "ProcessHub threw an exception, but an exception is not expected"
      listErrors.append(sResult)
   else:
      if EXPECTEDEXCEPTION not in sException:
         bSuccess = False
         sResult  = f"ProcessHub exception mismatch! Expected exception:\n'{EXPECTEDEXCEPTION}',\nbut thrown:\n'{sException}'"
         listErrors.append(sResult)

   if len(listErrors) == 0:
      bSuccess = True
      if EXPECTEDEXCEPTION is None:
         sResult  = "No exception thrown from ProcessHub (like expected)."
      else:
         sResult  = "ProcessHub threw expected exception."
   else:
      bSuccess = False
      sResult  = "ProcessHub did not throw expected exception."
...
try:
   from ProcessHub.version import VERSION as PHB_VERSION, VERSION_DATE as PHB_VERSION_DATE
   SUT_FULL_NAME = f"ProcessHub v. {PHB_VERSION} / {PHB_VERSION_DATE}"
...
for dictUsecase in listofdictUsecases:
   ...
   sException   = None
   try:
      # Execute the test file with the ProcessHub test utilities
      ...
      sException = f"'{reason}'"
      printerror(sException, "ProcessHub threw exception")
      oSelfTestLogFile.Write("ProcessHub threw exception:", 1)
      oSelfTestLogFile.Write(sException)
      oSelfTestLogFile.Write()

   # //////////////////////////////////////////////////////////////////////////////////////////////////////////////

   oSelfTestLogFile.Write("ProcessHub returned:", 1)
